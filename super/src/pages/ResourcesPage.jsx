...
/* ─────────────────────────────────────────────────────────────────
   RESTRUCTURATION selon le compte rendu de séance :
   "Restructurer la page Ressources en 5 sous-rubriques :
    Rapports / Guides / Manuels / Plaidoyer / Documentaires"
   ───────────────────────────────────────────────────────────────── */
...
const categories = [
  {
    ...
    items: [
      ...
      },
      {
        ...
        desc: "Bilan annuel des interventions, des projets et des résultats obtenus sur le terrain.",
        meta: 'PDF · Rapport annuel',
        path: null,
        comingSoon: true,
      },
      {
  ...
  },
  {
    ...
    label: 'Documentaires',
    icon: '🎬',
    title: 'Documentaires & audiovisuels',
    desc: "Films documentaires, capsules vidéo et contenus audiovisuels qui témoignent des actions de terrain.",
    items: [],
  },
]
...
export default function ResourcesPage() {
  ...
  const [activeCategory, setActiveCategory] = useState('rapports')

  // Synchroniser la catégorie active avec le hash de l'URL
  useEffect(() => {
  ...
  // Écouter les changements de hash (lorsque l'utilisateur clique sur un lien du header)
  useEffect(() => {
  ...
  return (
    <div className="space-y-16 pb-12 md:space-y-20">
      ...
      {/* ════════════════════════════════════════════════════════════
          ...
          ════════════════════════════════════════════════════════════ */}
      <section className="mx-auto max-w-7xl px-4 md:px-6 lg:px-8">
        {/* Ancres pour le scroll */}
        <div id="rapports" className="scroll-mt-24" />
        <div id="guides" className="scroll-mt-24" />
        <div id="manuels" className="scroll-mt-24" />
        <div id="plaidoyer" className="scroll-mt-24" />
        <div id="documentaires" className="scroll-mt-24" />
        ...
        <div className="flex flex-col items-start gap-4 md:flex-row md:items-end md:justify-between">
          ...
          <p className="shrink-0 text-xs font-medium text-slate-400">
            {current.items.length} ressource{current.items.length > 1 ? 's' : ''}
          </p>
        </div>
      ...
      {/* ════════════════════════════════════════════════════════════
          CTA BAS — demander une ressource
          ════════════════════════════════════════════════════════════ */}
      <section className="mx-auto max-w-7xl px-4 md:px-6 lg:px-8">
